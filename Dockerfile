...
RUN wget -O /tmp/akismet.zip https://downloads.wordpress.org/plugin/akismet.5.3.3.zip
RUN ls /tmp/
RUN unzip /tmp/akismet.zip -d /tmp/akismet
RUN mv /tmp/akismet/* /plugins

# https://wordpress.org/plugins/wp-stateless/
RUN wget -O /tmp/wp-stateless.zip https://downloads.wordpress.org/plugin/wp-stateless.4.0.4.zip
RUN ls /tmp/
