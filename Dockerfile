FROM alpine:3.23 as core-plugins
...
RUN mkdir -p tmp && curl https://downloads.wordpress.org/plugin/akismet.5.6.zip >> /tmp/akismet.zip
RUN ls /tmp/
RUN unzip /tmp/akismet.zip -d /tmp/akismet
RUN mv /tmp/akismet/* /plugins

# https://wordpress.org/plugins/wp-stateless/
RUN mkdir -p tmp && curl https://downloads.wordpress.org/plugin/wp-stateless.4.3.0.zip >> /tmp/wp-stateless.zip
RUN ls /tmp/
RUN unzip /tmp/wp-stateless.zip -d /tmp/wp-stateless
RUN mv /tmp/wp-stateless/* /plugins

FROM alpine:3.23 as core-themes
