FROM alpine:3.21 as core-plugins
...
RUN mkdir -p tmp && curl https://downloads.wordpress.org/plugin/akismet.5.3.5.zip >> /tmp/akismet.zip
RUN ls /tmp/
...
FROM alpine:3.21 as core-themes
