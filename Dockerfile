FROM alpine:3.20.3 as core-plugins
...
RUN mkdir -p tmp && curl https://downloads.wordpress.org/plugin/akismet.5.3.3.zip >> /tmp/akismet.zip
RUN ls /tmp/
RUN unzip /tmp/akismet.zip -d /tmp/akismet
RUN mv /tmp/akismet/* /plugins

# https://wordpress.org/plugins/wp-stateless/
RUN mkdir -p tmp && curl https://downloads.wordpress.org/plugin/wp-stateless.4.1.2.zip >> /tmp/wp-stateless.zip
RUN ls /tmp/
RUN unzip /tmp/wp-stateless.zip -d /tmp/wp-stateless
RUN mv /tmp/wp-stateless/* /plugins

FROM alpine:3.20.3 as core-themes
